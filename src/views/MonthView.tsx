...
import { daysOfMonthGrid, formatJaDate, formatJaYearMonth, today, yearMonthOf } from '../dates.js';
...
export function MonthView({ currentDate, blocksByDate, projects, projectById, onDayClick }: Props) {
  ...
  const monthAggregate = useMemo(() => aggregateMonthly(blocksByDate, ym), [blocksByDate, ym]);
  ...
  return (
    <div className="flex-1 overflow-auto flex flex-col bg-background">
      <ActualStrip
        ...
        projects={projects}
        emptyMessage={`${formatJaYearMonth(ym)}の登録なし`}
        label={`${formatJaYearMonth(ym)}の実績`}
      />
